...
public class DifferentSearches {
     ...
     */
    public void formatResults(NamingEnumeration namingEnumeration) throws Exception {
        int count = 0;
        try {
    ...
    public void formatAttributes(Attributes attrs) throws Exception {
        if (attrs == null) {
            ...
            try {
                for (NamingEnumeration namingEnumeration = attrs.getAll(); namingEnumeration.hasMore(); ) {
                    Attribute attrib = (Attribute) namingEnumeration.next();
